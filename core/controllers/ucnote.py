...
def uc_user_login(username,password,isuid=0,checkques=0,questionid='',answer='',ip=''):
    return uc_api_post("user","login",{'username':username,'password':password,"isuid":isuid,'checkques':checkques,'questionid':questionid,'answer':answer,'ip':ip})

def uc_app_ls():
    returns= uc_api_post('app','ls',{})
    return returns

def uc_feed_add(icon,uid,username,title_template='',title_data='',body_template='',body_data='',body_general='',target_ids='',images=[]):
    return uc_api_post('feed','add',{'icon':icon,'appid':UC_APPID,'uid':uid,'username':username,'title_template':title_template,'title_data':title_data,'body_template':body_template,'body_general':body_general,'target_ids':target_ids,'image_1':images[0]['url'],'image_1_link':images[0]['link'],'image_2':images[1]['url'],'image_2_link':images[1]['link'],'image_3':images[2]['url'],'image_3_link':images[2]['link'],'image_4':images[3]['url'],'image_4_link':images[3]['link']})

def uc_feed_get(limit=100,delete=True):
    returns=uc_api_post('feed','get',{'limit':limit,'delete':delete})
    return returns

def uc_user_register(username,password,email,questionid='',answer='',regip=''):
    returns=uc_api_post('user','register',{'username':username,'password':password,'email':email,'questionid':questionid,'answer':answer,'regip':regip})
    return returns
...
def uc_get_user(username,isuid=0):
    returns=uc_api_post('user','get_user',{'username':username,'isuid':isuid})
    return returns

def uc_user_merge(oldusername,newusername,uid,password,email):
    returns = uc_api_post('user','merge',{'oldusername':oldusername,'newusername':newusername,'uid':uid,'password':password,'email':email})
    return returns
...
def uc_user_logincheck(username,ip):
    return uc_api_post('user','logincheck',{'username':username,'ip':ip})


def uc_api_post(module,action,arg=[]):
    s = sep = ''
    for (k,v) in arg.items():
        ...
        if isinstance(v,(frozenset,list,set,tuple)):
            ...
            for k2, v2 in v:
                ...
                s2=s2+sep2+"{"+k+"}["+k2+"]="+urllib.urlencode(v2)
                sep2='&'
            s=s+sep+s2
        else:
    ...
    return uc_fopen2(url,500000,postdata,'',True,UC_IP,20)
def uc_fopen2(url,limit=0,post='',cookie='',bysocket=False,ip='',timeout=15,block=True):
    #todo add times parameter
    return uc_fopen(url,limit,post,cookie,bysocket,ip,timeout,block)
def uc_fopen(url,limit=0,post='',cookie='',bysocket=False,ip='',timeout=15,block=True):
...
def uc_api_input(data):
    s=urllib.quote_plus(uc_authcode(data+"&agent="+md5("Python-urllib/2.7")+"&time="+GetNowTime(),"ENCODE",UC_KEY))
    return s;
